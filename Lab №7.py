...
def run_algorithms():
    ...
    try:
        ...
        # Фильтруем точки, у которых хотя бы одна координата нечетная
        points = [(x, y) for x, y in points if x % 2 == 0 and y % 2 == 0]
    except:
    ...
    output_text.insert(tk.END, f"[Алгоритмический подход] Время: {alg_time:.4f} сек, Найдено: {len(alg_result)} подмножеств\n")
    ...
    output_text.insert(tk.END, f"[Оптимизированный подход] Время: {opt_time:.4f} сек, Найдено: {len(opt_result)} подмножеств\n")
...
points_entry.insert(0, "[(0,0), (1,1), (2,2), (0,1), (1,0), (2,0), (0,2), (1,2), (2,1), (3,3),(0,3), (1,3), (2,3), (3,0), (3,1), (3,2)]")
